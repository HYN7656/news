  ...
  <div class="list" v-bind:style="{ minHeight: offHeight + 'px' }">
    ...
    <div class="contain">
      ...
      <ul class="current_banner">
        <li v-for="i in typeList" :key="i.id" class="liStyle" :title="i.iName"><a @click="getSwit(i.id,i.iName)" class="banner_color" :class="{active:isActive == i.id}">{{i.name}}  <i class="icon iconfont icon-paly icon-postion"></i></a></li>
      </ul>
